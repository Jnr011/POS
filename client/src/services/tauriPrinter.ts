...
class TauriPrinterService {
  ...
  async listPrinters(): Promise<{ name: string; interface_type: string; identifier: string; status: string }[]> {
    if (!isTauri()) return [];
    try {
      const { list_thermal_printers } = await import('tauri-plugin-thermal-printer');
  ...
  async printReceipt(sale: Sale, storeInfo?: ReceiptStoreInfo): Promise<void> {
    ...
    const paymentMethod =
      ...
      sale.payment_method === 'card' ? 'Card' : 'Mobile Money';
    ...
    const sections = [
      ...
      // ── Totals ──
      tp.text(`Subtotal:${''.padStart(W - 9 - sale.total_price.toFixed(2).length)}₵${sale.total_price.toFixed(2)}`, { align: 'right' }),
      tp.text(`Tax:${''.padStart(W - 4 - sale.tax.toFixed(2).length)}₵${sale.tax.toFixed(2)}`, { align: 'right' }),
      tp.text(''),
      tp.text(`TOTAL:  ₵${sale.grand_total.toFixed(2)}`, { bold: true, size: 'double', align: 'right' }),
      tp.text(''),
